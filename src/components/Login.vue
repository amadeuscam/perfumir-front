<template>
  <div class="d-flex justify-content-center align-items-center flex-column login_background" style="height: 100vh">
    ...
    <form class="m-auto form-s" @submit.prevent="handleSubmit">
      ...
      <div class="mb-3">
        <input type="text" class="form-control" id="floatingInput" placeholder="juanito" required v-model="storage.username" />
      </div>
      <div class="mb-3">
        <label for="floatingPassword">Password</label>
        <input type="password" class="form-control" id="floatingPassword" placeholder="****" required v-model="storage.password" />
      </div>

      <button class="w-100 btn btn-lg btn-dark" type="submit">Sign in</button>
    </form>
    <router-link to="/register" class="btn btn-success btn-large my-3">¿You are not registered?</router-link>
  </div>
</template>
...
const handleSubmit = async () => {
  await customAxios
    ...
    .then((res) => {
      ...
      localStorage.setItem("current_user", decodeJwt(token));
      // this.$store.commit('username',sub)
      router.push("/ingredients").then(() => {
        router.go(0);
      });
    })
    .catch((error) => {
